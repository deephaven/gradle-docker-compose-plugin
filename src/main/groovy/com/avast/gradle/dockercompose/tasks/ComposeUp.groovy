...
class ComposeUp extends DefaultTask {
    ...
    protected Iterable<ServiceInfo> loadServicesInfo(Iterable<String> servicesNames) {
        ...
        Map<String, Iterable<String>> serviceToContainersIds = Collections.emptyMap();
        println(servicesNames.toString())
        while (servicesNames.size() != serviceToContainersIds.size() || serviceToContainersIds.values().stream().anyMatch({ it.isEmpty() })) {
            serviceToContainersIds = servicesNames.collectEntries { [(it): settings.composeExecutor.getContainerIds(it)] }
            println(serviceToContainersIds.dump())
        }
